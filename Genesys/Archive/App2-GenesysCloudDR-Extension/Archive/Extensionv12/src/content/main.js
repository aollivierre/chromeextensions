    ...
    chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
        try {
            ...
            if (message.action === 'environmentChange') {
                ...
                // Check for DR org ID directly - if found, always show DR badge
                const isDrOrgId = message.orgId === "d6154e9b-1f7a-40a4-9f06-e3a4c73fc4ac";
                if (isDrOrgId) {
                    log('DR organization ID detected - ensuring DR badge is shown');
                    import('./badge-ui').then(badgeModule => {
                        badgeModule.setDrOrgIdDetected(true);
                    });
                    
                    // Store minimal data to avoid storage quota issues
                    try {
                        saveStorageData({ 
                            environmentType: 'dr',
                            detectedOrgId: message.orgId 
                        });
                    } catch (error) {
                        log('Error saving DR org ID to storage (quota may be reached)');
                    }
                    
                    // Update badge to DR immediately
                    updateBadgeUI('dr');
                    sendResponse({ success: true, environment: 'dr' });
                    return;
                }
                
                // If not DR org ID, process normally
                if (message.orgId) {
                    try {
                        // Store minimal data to avoid quota issues
                        saveStorageData({ detectedOrgId: message.orgId });
                    } catch (error) {
                        log('Error saving org ID to storage (quota may be reached)');
                    }
                }
            ...
            // Handle direct badge updates (for tab activation)
            if (message.action === 'updateBadge' && message.environment) {
                log('Received direct badge update request:', message.environment);
                
                // Always check for DR org ID first
                const isDrOrgId = message.orgId === "d6154e9b-1f7a-40a4-9f06-e3a4c73fc4ac";
                if (isDrOrgId) {
                    log('DR organization ID detected with direct badge update - ensuring DR badge is shown');
                    import('./badge-ui').then(badgeModule => {
                        badgeModule.setDrOrgIdDetected(true);
                    });
                ...
                // For non-DR updates, check against stored data
                getStorageData(['detectedOrgId', 'environmentType']).then(data => {
                    // If we have the DR org ID stored, always prioritize it
                    if (data.detectedOrgId === "d6154e9b-1f7a-40a4-9f06-e3a4c73fc4ac") {
                        log(`DR org ID found in storage - ignoring update to ${message.environment}`);
                        import('./badge-ui').then(badgeModule => {
                            badgeModule.setDrOrgIdDetected(true);
                        });
                        updateBadgeUI('dr');
...
// Add badge persistence mechanism - but with much less frequent checking
setInterval(() => {
    // Get current environment first to check if we should show badge
    getStorageData(['environmentType']).then(data => {
        // Don't recreate badge for production environment
        if (data.environmentType === 'prod') {
            // Make sure badge is hidden for prod
            const existingBadge = document.getElementById('genesys-environment-badge');
            if (existingBadge) {
        ...
        // For non-prod environments, ensure badge is visible
