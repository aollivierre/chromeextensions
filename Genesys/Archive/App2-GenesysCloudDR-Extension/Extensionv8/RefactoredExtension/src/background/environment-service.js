...
class EnvironmentService {
    ...
    async checkUrlForEnvironment(tabId, url) {
        try {
            ...
            const data = await storageService.getMultiple(['environmentType', 'detectionMethod']);
    ...
    /**
     * Notify a specific tab about an environment change
     * @param {number} tabId - The tab ID to notify
     * @param {string} environment - The new environment
     * @param {string} orgId - The organization ID (optional)
     */
    notifyTab(tabId, environment, orgId = null) {
        // Cache the environment for this tab
        ...
        try {
            // For newly activated tabs, use updateBadge message
            // This allows for immediate badge updates when switching tabs
            chrome.tabs.sendMessage(tabId, {
                ...
                if (chrome.runtime.lastError) {
                    // Tab may not have content script loaded yet, try again with environmentChange message
                    log(`Content script might not be loaded in tab ${tabId}, trying environmentChange message`);
                    this.sendEnvironmentChangeMessage(tabId, environment, orgId);
                } else if (response && response.success) {
                    log(`Tab ${tabId} acknowledged badge update notification`);
                }
            });
        } catch (error) {
            log(`Error notifying tab ${tabId}:`, error);
            // Try with standard message as fallback
            this.sendEnvironmentChangeMessage(tabId, environment, orgId);
        }
    }
