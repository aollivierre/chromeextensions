#
# Genesys Environment Differentiation for Chrome - Fixed Version
# 
...
$manifestJson = @{
    ...
    "content_scripts" = @(
        @{
            "matches" = @("https://apps.dr1.pure.cloud/*")  # Correct DR environment URL
            "css" = @("dr-style.css")
            "all_frames" = $true
...
# Create CSS file with stronger visual styling - more prominent banner with no white space
$cssContent = @"
...
$Shortcut.Arguments = "--app=https://apps.dr1.pure.cloud:443 --force-dark-mode --user-data-dir=""$userDataDir"" --load-extension=""$extensionFolder"" --no-first-run"
